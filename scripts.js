function TodoItemFormatter() {
    this.formatTask = function(task) {
      return task.length > 14 ? task.slice(0, 14) + "..." : task;
    }
  
    this.formatDueDate = function(dueDate) {
      return dueDate || "No due date";
    }
  
    this.formatStatus = function(completed) {
      return completed ? "Completed" : "Pending";
    }
  }
  

  function TodoManager(todoItemFormatter) {
    this.todos = JSON.parse(localStorage.getItem("todos")) || [];
    this.todoItemFormatter = todoItemFormatter;
  
    this.addTodo = function(task, dueDate) {
      const newTodo = {
        id: this.getRandomId(),
    ...
    this.editTodo = function(id, updatedTask) {
    ...
    this.toggleTodoStatus = function(id) {
      const todo = this.todos.find((t) => t.id === id);
      if (todo) {
    ...
    this.filterTodos = function(status) {
      switch (status) {
        case "all":
          return this.todos;
        case "pending":
  ...
  function UIManager(todoManager, todoItemFormatter) {
    ...
    this.addEventListeners = function() {
      ...
      this.deleteAllBtn.addEventListener("click", this.handleClearAllTodos.bind(this));
    ...
    this.handleClearAllTodos = function() {
    ...
    this.handleEditTodo = function(id) {
    ...
    this.handleToggleStatus = function(id) {
